...
def prepare_data_for_model(train_features, test_features):
    ...
    comps_tensor_first_part = torch.FloatTensor(X_train_scaled).unsqueeze(1)
    comps_tensor_vectors = torch.zeros(batch_size_train, MAX_NUM_TRANSFORMATIONS, MAX_TAGS)
    # Adjust third_part to match expected_input_size - input_size - lstm_embedding_size * 2
    third_part_size = expected_input_size - input_size - (lstm_embedding_size * 2 * num_layers)
    comps_tensor_third_part = torch.zeros(batch_size_train, 1, third_part_size if third_part_size > 0 else 1)
    ...
    comps_tensor_third_part_test = torch.zeros(batch_size_test, 1, third_part_size if third_part_size > 0 else 1)
