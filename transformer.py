...
def process_halide(halide_data):
    ...
    for node in nodes:
        ...
        sequence.append(np.array(type_onehot + specific_features))
    ...
    specific_features = [0.0] * 37 + normalize_numerical([exec_time], 3)
    sequence.append(np.array(type_onehot + specific_features))
    ...
    return np.array(sequence), exec_time
...
def process_tiramisu(tiramisu_data):
    ...
    for sched in schedules:
        ...
        for it_name, it_data in iterators.items():
            ...
            specific_features.extend([0.0] * 28)  # Placeholders
            sequence.append(np.array(type_onehot + specific_features))
        ...
        type_onehot = [0.0, 0.0, 1.0, 0.0]
        specific_features = [0.0] * 37 + normalize_numerical([mean_exec_time], 3)
        sequence.append(np.array(type_onehot + specific_features))
