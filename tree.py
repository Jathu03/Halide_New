...
# Function to create tree representation from JSON
def create_tree_representation(json_data):
    root = TreeNode("root", "root")
    
    def build_tree(node_data, parent):
        node_type = node_data.get('type', 'unknown')
        if not node_type or node_type == 'unknown':
            node_type = 'global' if node_data.get('name') == 'Global Features' else 'operation'
        
        node = TreeNode(node_data.get('name', 'unnamed'), node_type, node_data)
...
# Function to extract tree-specific features
...
def process_tree_output_directory(main_dir):
    ...
    for root, dirs, files in os.walk(main_dir):
        if 'tree_representation.json' in files:
            ...
            try:
                ...
                features, tree = extract_features(json_data)
    ...
    combined = list(zip(all_features, all_trees, file_names))
    random.shuffle(combined)
    all_features, all_trees, file_names = zip(*combined)
    
    test_size = min(50, len(all_features))
    train_features = all_features[:-test_size]
    test_features = all_features[-test_size:]
    train_trees = all_trees[:-test_size]
    test_trees = all_trees[-test_size:]
...
def main(main_dir):
    ...
    train_features, test_features, test_file_names, train_trees, test_trees = process_tree_output_directory(main_dir)
    ...
    (train_sequences, train_scalar, y_train,
     ...
     y_scaler, seq_input_size, scalar_input_size, feature_columns) = prepare_data_for_model(
        train_features, test_features, train_trees, test_trees
    )
