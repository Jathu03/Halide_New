...
int main(int argc, char* argv[]) {
    try {
        ...
        if (torch::cuda::is_available()) {
            ...
            // Get current device index - using the proper API
            int device_index = 0;  // Default to device 0
            std::cout << "Using CUDA device: " << device_index << std::endl;
            
            // Note: We've removed the calls to functions that don't exist in the C++ API
            // If you need to empty the cache, you would need to use a different approach
            // or include the appropriate headers
            
            // Display device information if available
            std::cout << "GPU information: Using CUDA-capable device" << std::endl;
        } else {
