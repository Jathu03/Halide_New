...
# Enhanced feature extraction with additional derived features
def extract_features(json_data):
    ...
    features['bytes_per_vector'] = safe_div(features.get('sched_bytes_at_realization', 0), features.get('sched_num_vectors', 1))
    ...
    features['node_edge_ratio'] = safe_div(nodes_count, edges_count + 1)
    features['nodes_per_schedule'] = safe_div(nodes_count, features.get('scheduling_count', 1))
    features['op_diversity'] = len([k for k, v in features.items() if k.startswith('op_') and v > 0])
    
    # Additional derived features
    features['cache_hit_ratio'] = safe_div(features.get('cache_hits', 0), features.get('cache_hits', 0) + features.get('cache_misses', 1))
    features['vector_efficiency'] = safe_div(features.get('sched_num_vectors', 0), features.get('sched_vector_size', 1))
    
    # Create fixed-length feature vector
    fixed_features = {key: features.get(key, 0.0) for key in FIXED_FEATURES + ['cache_hit_ratio', 'vector_efficiency']}
...
# Enhanced data preparation with adaptive scaling
def prepare_data_for_model(train_features, test_features):
    FIXED_FEATURES_EXTENDED = FIXED_FEATURES + ['cache_hit_ratio', 'vector_efficiency']
    
    train_sequences = [np.array([[features.get(key, 0.0) for key in FIXED_FEATURES_EXTENDED]]) for features in train_features]
    ...
    for feature in skewed_features:
        if feature in train_scalar_df.columns:
            skewness = train_scalar_df[feature].skew()
            if skewness > 1:  # Only apply log transform for highly skewed features
    ...
    for i in range(len(train_features)):
        ...
        for _ in range(augment_count):
            ...
            # Add feature-specific noise for important features
            seq_aug = train_sequences_padded[i] + noise_seq
            scalar_aug = train_scalar_scaled[i] + noise_scalar
            
            if cache_hits_idx != -1:
                scalar_aug[cache_hits_idx] += np.random.normal(0, 0.05)
            if bytes_rate_idx != -1:
    ...
    # Save scalers
    with open('scaler_X_scalar.pkl', 'wb') as f:
        pickle.dump(scaler_X_scalar, f)
...
# Enhanced custom loss function
def custom_loss(outputs, targets, scalar_inputs, feature_indices, feature_importances, huber_delta=0.5, mae_weight=0.25, l1_lambda=5e-6):
...
def train_model(model, train_loader, test_loader, criterion, optimizer, feature_indices, feature_importances, num_epochs=1500, patience=75, accumulation_steps=4, checkpoint_path='recursive.pth'):
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=50, T_mult=2, eta_min=5e-7)
...
def main(main_dir):
    ...
    feature_importances = {
        ...
        'cache_hit_ratio': 0.0300,
        'vector_efficiency': 0.0200
    }
    ...
    train_losses, val_losses = resume_training(
        ...
        num_epochs=1500, patience=75, accumulation_steps=4, checkpoint_path='recursive.pth'
    )
