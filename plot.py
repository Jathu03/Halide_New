...
def evaluate_model(model, X_test_seq, X_test_scalar, y_test, y_scaler, file_names_test, feature_importances):
    ...
    # Filter out high values (e.g., 95th percentile)
    percentile_threshold = np.percentile(y_test_actual, 95)
    mask = (y_test_actual <= percentile_threshold) & (y_pred_actual <= percentile_threshold)
    y_test_actual_filtered = y_test_actual[mask]
    y_pred_actual_filtered = y_pred_actual[mask]
    ...
    # Separate plots for Feature Importance, Scatter Plot, and Error Distribution
    # Feature Importance
    sorted_features = dict(sorted(feature_importances.items(), key=lambda x: x[1], reverse=True))
    plt.figure(figsize=(10, 6))
    sns.barplot(x=list(sorted_features.values()), y=list(sorted_features.keys()), palette='viridis')
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(y_test_actual_filtered, y_pred_actual_filtered, alpha=0.6, color='blue', label='Predictions')
    min_val, max_val = min(y_test_actual_filtered.min(), y_pred_actual_filtered.min()), max(y_test_actual_filtered.max(), y_pred_actual_filtered.max())
    ...
    # Error Distribution
    errors = y_test_actual_filtered - y_pred_actual_filtered
    plt.figure(figsize=(10, 6))
    ...
    # Remaining visualizations in a 2x3 grid (Residual Plot, Cumulative Error, Time Series)
    ...
    axes[0, 0].scatter(y_test_actual, residuals, alpha=0.6, color='green')
    axes[0, 0].axhline(0, color='red', linestyle='--')
    axes[0, 0].set_xlabel('Actual Execution Time (ms)')
    axes[0, 0].set_ylabel('Residual (Actual - Predicted) (ms)')
    ...
    axes[0, 1].plot(sorted_errors, cumulative, color='darkblue')
    ...
    # Actual vs. Predicted (Sequential)
    axes[0, 2].plot(range(len(y_test_actual)), y_test_actual, label='Actual', color='blue')
    axes[0, 2].plot(range(len(y_pred_actual)), y_pred_actual, label='Predicted', color='orange', linestyle='--')
    axes[0, 2].set_xlabel('Test Sample Index')
    axes[0, 2].set_ylabel('Execution Time (ms)')
    axes[0, 2].set_title('Actual vs. Predicted (Sequential)')
    axes[0, 2].legend()
    axes[0, 2].grid(True)
...
def main(main_dir):
    ...
    y_test_actual, y_pred_actual = evaluate_model(
        ...
        y_scaler, test_file_names, feature_importances
    )
...
if __name__ == "__main__":
    main_dir = "/home/kowrisaan/jathu/Halide_New/Tree_Output"
    random.seed(42)
