...
def analyze_feature_importance(features_list, output_dir):
    ...
    print(f"Model saved to {output_dir}/random_forest_model.joblib")
...
def predict_custom_input(model, scaler, feature_names, input_file=None, input_features=None):
    ...
    if input_file:
        ...
        features = input_features.copy()  # Avoid modifying the input dictionary
    
    # Extract actual execution time if available (for validation)
...
def main(main_dir="Graph_Output", output_dir="analysis_results", custom_input_file=None, custom_input_features=None):
    ...
    if custom_input_file or custom_input_features:
        ...
        try:
            ...
            print(f"Predicted Execution Time: {result['predicted_time_ms']:.2f} ms")
            if 'actual_time_ms' in result:
                print(f"Actual Execution Time: {result['actual_time_ms']:.2f} ms")
            ...
            with open(f"{output_dir}/feature_importance_report.txt", 'a') as f:
                ...
                f.write(f"Predicted Execution Time: {result['predicted_time_ms']:.2f} ms\n")
                if 'actual_time_ms' in result:
                    f.write(f"Actual Execution Time: {result['actual_time_ms']:.2f} ms\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output-dir', default='analysis_results', help='Output directory for reports and models')
