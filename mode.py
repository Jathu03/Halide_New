...
# Process Graph_Output directory for training
...
# Prepare single file for inference
def prepare_single_file(file_path, scaler_X_seq, scaler_X_scalar, seq_input_size, scalar_input_size, feature_columns):
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            json_data = json.load(f)
        features = extract_features(json_data)
        if features is None:
            print(f"Error: Invalid features extracted from {file_path}")
            return None
        ...
        # Align columns with training feature_columns
        missing_cols = [col for col in feature_columns if col not in scalar_df.columns]
        for col in missing_cols:
            scalar_df[col] = 0
        scalar_df = scalar_df[feature_columns]
        ...
        return sequence_padded, scalar_tensor, features.get('execution_time_ms', None)
    except Exception as e:
...
def perform_inference(model, file_path, y_scaler, scaler_X_seq, scaler_X_scalar, seq_input_size, scalar_input_size, feature_columns):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.to(device)
    model.eval()
    
    result = prepare_single_file(file_path, scaler_X_seq, scaler_X_scalar, seq_input_size, scalar_input_size, feature_columns)
    if result is None:
        return None
    
    seq_input, scalar_input, actual_time = result
    seq_input, scalar_input = seq_input.to(device), scalar_input.to(device)
    
    with torch.no_grad():
        pred_scaled = model(seq_input, scalar_input)
    
    pred_scaled = pred_scaled.cpu().numpy()
    pred_transformed = y_scaler.inverse_transform(pred_scaled)
    pred_actual = np.expm1(pred_transformed)[0][0]
    pred_actual = max(pred_actual, 0)
    
    print(f"\nInference for {file_path}:")
    print(f"  Predicted execution time: {pred_actual:.2f} ms")
    if actual_time is not None:
        print(f"  Actual execution time: {actual_time:.2f} ms")
        print(f"  Error percentage: {abs(actual_time - pred_actual) / actual_time * 100:.2f}%")
    
    return pred_actual, actual_time
...
def train(main_dir):
    ...
    return model, y_scaler, scaler_X_seq, scaler_X_scalar, seq_input_size, scalar_input_size, feature_columns, y_test_actual, y_pred_actual

# Inference function
def inference(file_path, model_path='model.pt'):
    try:
        # Load model metadata
        with open('model_metadata.json', 'r') as f:
            metadata = json.load(f)
        seq_input_size = metadata['seq_input_size']
        scalar_input_size = metadata['scalar_input_size']
        feature_columns = pd.Index(metadata['scalar_features'])
        ...
        # Perform inference
        pred, actual = perform_inference(
            model, file_path, y_scaler, scaler_X_seq, scaler_X_scalar,
            seq_input_size, scalar_input_size, feature_columns
        )
        return pred, actual
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description='LSTM Model for Training or Inference')
    parser.add_argument('--mode', choices=['train', 'inference'], required=True, help='Mode: train or inference')
    parser.add_argument('--main-dir', type=str, default='/home/kowrisaan/jathu/Halide_New/Graph/Graph_Output', help='Main directory for training data')
    parser.add_argument('--file-path', type=str, help='Path to JSON file for inference')
    parser.add_argument('--model-path', type=str, default='model.pt', help='Path to saved model for inference')
    ...
    if args.mode == 'train':
        result = train(args.main_dir)
        if result is None:
            print("Training failed. Check logs and data directory.")
    elif args.mode == 'inference':
        if not args.file_path:
            print("Error: --file-path is required for inference mode")
            return
        ...
        if result is None:
            print("Inference failed. Check model and input file.")
